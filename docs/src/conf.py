"""Sphinx configuration for safl.dk."""
...
# like ``[section](#section)`` resolve without a manual ``(label)=`` on
# every heading.
...
# HTML output: the custom 8-bit theme lives under _themes/blue8bit and is
# kept as-is; only the source format (reST -> Markdown) and the Sphinx/MyST
# toolchain changed.
...
# Copy the custom-domain marker verbatim into the build root so GitHub
# Pages keeps serving safl.dk. html_extra_path files land at the output
# top level (unlike html_static_path, which nests under _static/).
